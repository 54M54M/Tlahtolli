<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="$emit('close')">
        <div class="w-full max-w-md bg-[#123456] rounded-lg p-6 shadow-xl">
            ...
            <div class="flex flex-col items-center mb-6">
                <div class="relative">
                    ...
                    <button class="absolute bottom-0 right-0 bg-[#58CC02] hover:bg-[#4BB302] rounded-full p-1.5 transition-colors"
                        title="Cambiar foto (próximamente)" disabled>
                ...
                <div class="flex gap-3">
                    <div class="flex-1">
                        ...
                        <input v-model="form.firstName" type="text"
                            class="w-full bg-[#0A2136] border border-gray-600 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-[#58CC02] transition-colors" />
                    </div>
                    <div class="flex-1">
            ...
            <div v-if="errorMsg" class="mt-4 p-3 bg-red-900/40 border border-red-600 rounded-lg text-red-300 text-sm">
                {{ errorMsg }}
            </div>
            <div v-if="successMsg" class="mt-4 p-3 bg-green-900/40 border border-green-600 rounded-lg text-green-300 text-sm">
                {{ successMsg }}
            </div>
            ...
            <div class="flex gap-3 mt-6">
                ...
                <button @click="save" :disabled="loading"
                    class="flex-1 bg-[#58CC02] hover:bg-[#4BB302] disabled:opacity-50 text-white font-semibold py-2.5 rounded-lg transition-colors">
...
const save = async () => {
    ...
    try {
        ...
        setTimeout(() => emit('updated', updated), 800)
    } catch (err) {
